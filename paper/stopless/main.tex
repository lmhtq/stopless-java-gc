...
capabilities break reference identity), and we verify that the hardware
primitive that closes the window, the per-page capability-load
generation barrier, works under our stack: opening a revocation epoch
costs milliseconds and lazily revokes on first capability load. What we build and measure is the
...
\item \textbf{Measurement} (\S\ref{sec:eval}): with the root set held
  ...
  at batch size 8 (workload-constrained stop-the-world configuration). Healing a trapped reference costs $\sim$20\,\textmu s of in-handler
...
\paragraph{Feasibility of the sound design.} The instrument that
closes the window exists in our stack, and we verified it works under
QEMU emulation: CheriBSD's load-side revocation drives Morello's
per-page capability-load generation (CLG) barrier, and a minimal probe
shows that (i)~\emph{opening} a revocation epoch---after which no
stale capability can be loaded with its tag intact---costs 3.7\,ms
...
16\,MiB capability-dense region); (iii)~the kernel API additionally offers a worker-thread vmspace scan
(\texttt{CHERI\_REVOKE\_ASYNC}; the probe confirmed only its flag
constraints---it is rejected in combination with the closing
flag---so asynchronous operation is an API capability we have not yet
exercised).
The sound batched design follows: relocate and mark each cycle;
\emph{open} the epoch inside the pause (milliseconds,
heap-independent); run the heap-linear scan asynchronously off-pause.
The mutator pause becomes relocation plus epoch-open---both
heap-independent---which is the Cornucopia Reloaded result transplanted
to a moving collector. Building this is the immediate next step.
...
away $10.3\times$ in a controlled configuration because the sweep's
cost is approximately independent of how much it revokes---with the
per-page load-barrier instrument that makes batching sound verified
working under our stack. The capability-specific lessons---integer-only GC
