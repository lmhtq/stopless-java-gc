...
cycle pause by $10.3\times$ at batch size 8 in a controlled
stop-the-world configuration---though we show the batch window is
...
\item \textbf{Measurement} (\S\ref{sec:eval}): with the root set held
  ...
  cost; batching amortises the mean cycle pause $10.3\times$ at batch
  size 8 (controlled stop-the-world configuration). Healing a trapped reference costs $\sim$20\,\textmu s of in-handler
...
\caption{Batched revocation, \textsf{IntegrityGC}, controlled
stop-the-world configuration (64 benchmark-driven cycles after
bootstrap, fixed implementation; all integrity verifications pass at
every batch size). (a)~Per-cycle pause, log scale: with batching most
cycles pause only for relocation ($\sim$11\,ms floor); the sweep cost
concentrates in every $N$th cycle. (b)~Mean pause per cycle falls from
1.57\,s to 0.15\,s ($10.3\times$) at batch size 8. The worst-case
(sweep) cycle is unchanged---soundly moving it off-pause is the CLG
design measured feasible in \S\ref{sec:eval:batch}.}
\label{fig:batch}
\end{figure}
...
continues every cycle. We measure this in a \emph{controlled
stop-the-world configuration}---64 collection cycles driven explicitly
by the benchmark after VM bootstrap, on the fixed (transitive-chase)
implementation---so no identity-sensitive code executes inside the
batch window. Figure~\ref{fig:batch}: mean pause per cycle falls
$1.57 \to 0.28 \to 0.15$\,s for batch sizes 1, 4, 8---a
$10.3\times$ reduction, tracking the sweep count (64/16/8), with all
64 integrity verifications passing deterministically at every batch
size. The per-cycle trace shows the expected shape: a floor at the
heap-independent relocation pause ($\sim$11\,ms), with the sweep cost
concentrated in periodic spikes whose amplitude is \emph{unchanged}
(max 1.2--2.6\,s in all three configurations): batching improves the
mean, never the worst case.

The same mechanism run \emph{concurrently} demonstrates the window
unsoundness of \S\ref{sec:design:batch} empirically: batch ${>}1$
with the background collector fails nondeterministically when cycles
overlap identity-sensitive VM bootstrap (the both-tagged fast path
judges old and new copies unequal; the typical symptom is an
\texttt{IllegalAccessError} out of \texttt{java.lang.invoke}), and
across repeated trials fails more often than it completes. Batched
revocation is sound only when the window is closed to
identity-sensitive and mutating code.
...
16\,MiB capability-dense region); (iii)~the closing scan can run on a
kernel worker thread (\texttt{CHERI\_REVOKE\_ASYNC}; mutually
exclusive with the closing flag, so open asynchronously, close later).
