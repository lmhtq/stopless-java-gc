...
growing to 82\,MiB the mutator pause is median 18.6\,ms (p90 33\,ms)
while the closing scan grows to 6.3\,s off-pause. What we build and measure is the
...
\item \textbf{Measurement} (\S\ref{sec:eval}): with the root set held
  ...
  \emph{sound}: median pause 18.6\,ms over a heap growing to
  82\,MiB, with the heap-linear scan off-pause. Healing a trapped reference costs $\sim$20\,\textmu s of in-handler
...
\S\ref{sec:eval:batch} quantify it; making it concurrent (as
Cornucopia Reloaded does for C heaps) is the obvious next step and is
orthogonal to the relocation path measured in panel~(a). By contrast the sweep itself, panel~(b), is linear in
...
improvement---moving the sweep off-pause is the CLG design assessed
feasible in \S\ref{sec:eval:batch}.}
\label{fig:batch}
\end{figure}
...
allocation benchmark growing the heap $0.3\to82$\,MiB; 345 cycles,
one trace. (a)~Mutator pause (relocation + epoch-open), log scale:
median 18.6\,ms, p90 33\,ms across the whole range; the tail (max
1.5\,s, $\sim$1 cycle in 10 late in the run) is the epoch-open's
page-table component, which grows with pages touched during the
previous closing scan. (b)~The heap-linear closing scan
($0.6\to6.3$\,s) runs on the collector thread with mutators live.
For comparison, the synchronous design pays panel~(b) \emph{inside}
panel~(a)'s pause.}
\label{fig:sound}
\end{figure}
...
with mutators running under the CLG barrier. Because every cycle's
marks are published before mutators resume, there is no window in
which a stale capability can be loaded tagged---both unsoundness modes
of \S\ref{sec:design:batch} are closed by construction.

Empirically the previously-failing configurations now pass: the
concurrent \textsf{IntegrityGC} runs that failed under batching
completed 3 of 3 trials with full verification (plus 2 of 2
\textsf{ConcatTest}), and Figure~\ref{fig:sound} shows the pause
structure over a heap growing $0.3\to82$\,MiB: the mutator pause is
\emph{median 18.6\,ms, p90 33\,ms}---the epoch-open contributes a
median 3.7\,ms---while the closing scan grows $0.6\to6.3$\,s
\emph{off-pause}. At the large end the synchronous design would pay
several seconds inside every pause; the sound mode pays tens of
milliseconds typically. We disclose the tail honestly: occasional
opens reach 1.5\,s, consistent with the open's page-table (LCLG
update) work scaling with pages the mutator touched during the
previous, heap-linear close; bounding this tail (incremental LCLG
update, or the kernel worker-thread scan) is the next engineering
...
median mutator pause to 18.6\,ms on a heap where the synchronous
