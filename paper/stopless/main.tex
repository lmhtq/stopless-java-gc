...
it revokes, batching relocations across collection cycles amortises the
mean cycle pause by $11.9\times$---though we show the batch window is
unsound for general programs (stale-but-still-tagged capabilities break
reference identity), so this is the mechanism's amortisation potential,
not a deployable configuration. What we build and measure is the
...
\item \textbf{Measurement} (\S\ref{sec:eval}): with the root set held
  ...
  size 32. Healing a trapped reference costs $\sim$20\,\textmu s of in-handler
...
0.84\,s to 0.07\,s ($11.9\times$) at batch size 32. (Data from runs
that completed; batch ${>}1$ is not a sound configuration---see text.)}
\label{fig:batch}
\end{figure}
...
(62/23/4 sweeps over the respective runs). The shown runs completed and
passed their structure-and-identity verification, but the window
unsoundness of \S\ref{sec:design:batch} is real, not hypothetical:
across repeated trials, batch ${>}1$ under the concurrent collector
fails nondeterministically when cycles overlap identity-sensitive VM
bootstrap (the both-tagged fast path judges old and new copies unequal;
the typical symptom is an \texttt{IllegalAccessError} out of
\texttt{java.lang.invoke}), and fails more often than it completes.
The $11.9\times$ figure is therefore the measured amortisation
potential a sound window-closing design (per-page load barrier) would
inherit, not a configuration that runs general programs today. The
per-cycle trace shows the
